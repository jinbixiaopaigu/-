...
#define PROID		"STM32&ifmhmtZTe0e"       //设置连接服务器的名称

#define AUTH_INFO	"18845a7e97ef6019a654daab39de6a70595a48d5de050c324aa37905d8960abe"		//设置连接服务器的密码

#define DEVID		"ifmhmtZTe0e.STM32|securemode=2,signmethod=hmacsha256,timestamp=1693883157163|"		//设置连接服务器的ID号

extern u8 FlagCA;   //报警标志
extern u8 Heart;       //心率阈值
extern u8 Bloodox;       //血氧阈值
extern float TempY;       //体温阈值
extern int Bloodpr;       //血压阈值
extern int nor1,nor2,nor3,nor4,nor5;  //报警 1.跌倒报警    1：异常  0：正常
		...
		case MQTT_PKT_PUBLISH:														//接收的Publish消息
			...
			if(result == 0)
			{
				...
				if(dataPtr != NULL && result != -1)					//如果找到了
				{
					...
					if(strstr((char *)req_payload, "Cancell"))		//搜索"LED"   灯光
					{
						printf("Cancell = %d\r\n", num);     //num LED_K:V 中V的值
		          ...
		          nor4=0;
					...
					//接收阈值设定
					else if(strstr((char *)req_payload, "Heart"))	//温度阈值
					{
						printf("Heart = %d\r\n", num);
						
						Heart=num;
					}
					else if(strstr((char *)req_payload, "Bloodox"))	//湿度阈值
					{
						printf("Bloodox = %d\r\n", num);
						
						Bloodox=num;
					}
					else if(strstr((char *)req_payload, "Temp"))	//光照度阈值
					{
						printf("Temp = %d\r\n", num);
						
						TempY=num;
					}
					else if(strstr((char *)req_payload, "Bloodpr"))	//降雨量阈值
					{
						printf("Bloodpr = %d\r\n", num);
